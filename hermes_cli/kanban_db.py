...
def _default_spawn(
    ...
    *,
    ...
    # Defense-in-depth: skip skills that don't resolve for the worker's
    # profile home. ``create_task`` rejects unresolvable skills at create
    # time, but legacy rows persisted before that gate landed (or skills
    # uninstalled between create and dispatch) would otherwise still crash
    # the worker at CLI startup. Logging the skip to the worker log keeps
    # the drop auditable without tripping the circuit breaker.
    if task.skills:
