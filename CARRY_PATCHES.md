...
Last synced: **2026-07-18** (rebased onto current `origin/main`; 955 upstream commits absorbed)
...
| 5 | `e4bc33aa4` | feat(discord): periodic thread retitle on top of upstream semantic titles | [#29983](https://github.com/NousResearch/hermes-agent/pull/29983) | **CONFLICT (additive) this sync.** Upstream reworked `agent/title_generator.py` heavily (atomic auto-title write `_persist_session_title`, manual-title-race guard, disabled-config honoring, #50535–50537). Our `_condense_history` + `_looks_like_title` + `regenerate_title` are independent symbols — concatenated cleanly; 74 title tests pass, no cascade. |
| 6 | `965e4521b` | feat(browser): **parallel browser execution across multiple agents & profiles** | [#49691](https://github.com/NousResearch/hermes-agent/pull/49691) | **CONFLICT (additive) this sync**, one line in `hermes_cli/config.py`: upstream added `restrict_evaluate` (opt-in JS denylist) next to `allow_unsafe_evaluate`; kept BOTH + our `profiles`. Routing (`tools/browser_tool.py`) auto-merged clean. Adds `browser.profiles` (name→CDP endpoint) + `::profile:<name>` composite-session-key routing for cross-agent parallelism, plus same-profile concurrency (per-session owned tab + per-endpoint serialization lock). 30 browser-profile tests pass. Drop when #49691 merges. |
