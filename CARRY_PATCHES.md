...
| `4df1dda3edc` | fix(browser): never send close to an attached CDP browser | none (local-only, upstream bug) | low | `_cleanup_single_browser_session` skips the agent-browser `close` for sessions flagged `cdp_override` — Hermes closes only browsers it launched. Pre-existing upstream bug, not caused by the profiles carry; affects plain `browser.cdp_url` users too. Candidate to upstream. |
