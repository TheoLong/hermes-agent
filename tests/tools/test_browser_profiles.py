...
import tools.browser_tool_lifecycle as _lifecycle_mod
...
def _reset_state(monkeypatch):
    ...
    monkeypatch.setattr(_lifecycle_mod, "_start_browser_cleanup_thread", lambda: None)
    monkeypatch.setattr(_lifecycle_mod, "_update_session_activity", lambda t: None)
    monkeypatch.setattr(browser_tool, "_is_camofox_mode", lambda: False)
    # No global override / cloud provider unless a test sets one.
    monkeypatch.setattr(_cdp_mod, "_get_cdp_override", lambda: "")
    monkeypatch.setattr(_cloud_mod, "_get_cloud_provider", lambda: None)
...
def _set_profiles(monkeypatch, mapping):
    ...
    monkeypatch.setattr(_cdp_mod, "_resolve_cdp_override", lambda u: u)
...
class TestSessionCreationRouting:
    ...
    def test_profile_key_creates_cdp_session_for_its_endpoint(self, monkeypatch):
        ...
        monkeypatch.setattr(_cdp_mod, "_ensure_cdp_supervisor", lambda t: None)

        key = browser_tool._compose_profile_session_key("default", "work")
        session = _session_mod._get_session_info(key)
    ...
    def test_two_profiles_get_independent_sessions(self, monkeypatch):
        ...
        sa = _session_mod._get_session_info(ka)
        sb = _session_mod._get_session_info(kb)
    ...
    def test_profile_session_skips_cloud_provider(self, monkeypatch):
        ...
        monkeypatch.setattr(_cloud_mod, "_get_cloud_provider", lambda: provider)
...
class TestBackwardsCompat:
    ...
    def test_legacy_cdp_url_still_resolves_as_default(self, monkeypatch):
        ...
        monkeypatch.setattr(browser_tool, "_get_browser_profiles", lambda: {})
        monkeypatch.setattr(_cdp_mod, "_get_cdp_override", lambda: "ws://localhost:9224/legacy")
...
class TestCleanupFanOut:
    ...
    def test_bare_task_cleanup_reaps_profile_sessions(self, monkeypatch):
        reaped = []
        monkeypatch.setattr(_lifecycle_mod, "_cleanup_single_browser_session", lambda k: reaped.append(k))
        monkeypatch.setattr(browser_tool, "_active_sessions", {
        ...
        _lifecycle_mod.cleanup_browser("default")
    ...
    def test_explicit_profile_key_reaps_only_itself(self, monkeypatch):
        ...
        _lifecycle_mod.cleanup_browser("default::profile:work")
...
class TestOwnedTab:
    ...
    def test_acquire_failure_returns_none(self, monkeypatch):
        monkeypatch.setattr(
            _session_mod, "_run_raw_agent_browser",
...
class TestSameProfileCommandRouting:
    ...
    def _stub_command_env(self, monkeypatch):
        ...
        monkeypatch.setattr(_install_mod, "_find_agent_browser", lambda: "agent-browser")
        monkeypatch.setattr(_install_mod, "_requires_real_termux_browser_install", lambda c: False)
        monkeypatch.setattr(_cloud_mod, "_is_local_mode", lambda: False)
        monkeypatch.setattr(_cloud_mod, "_get_browser_engine", lambda: "auto")
        monkeypatch.setattr(browser_tool, "_build_browser_env", lambda: {})
        monkeypatch.setattr(_install_mod, "_merge_browser_path", lambda p: p)
        monkeypatch.setattr(browser_tool, "_socket_safe_tmpdir", lambda: "/tmp")
        monkeypatch.setattr(_lifecycle_mod, "_write_owner_pid", lambda d, n: None)
        monkeypatch.setattr(_session_mod, "_needs_chromium_sandbox_bypass", lambda: False)
        monkeypatch.setattr(_lp_mod, "_lightpanda_fallback_reason", lambda *a: None)
    ...
    def test_profile_command_activates_owned_tab_before_running(self, monkeypatch):
        ...
        monkeypatch.setattr(_session_mod, "_get_session_info",
        ...
        _session_mod._run_browser_command("s::profile:work", "snapshot")
    ...
    def test_non_profile_command_skips_tab_machinery(self, monkeypatch):
        self._stub_command_env(monkeypatch)
        raw_calls = []
        monkeypatch.setattr(_session_mod, "_run_raw_agent_browser",
        ...
        _session_mod._run_browser_command("bare-task", "snapshot")
